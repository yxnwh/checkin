...
function check() {
    return new Promise(async (resolve) => {
        try {
            let url = "https://aiqicha.baidu.com/usercenter/getBenefitStatusAjax";
            let res = await axios.get(url, header);
            if res.json()["data"]["AQ03008"] == true:
	       notice += "爱企查京东e卡有货，请进行兑换"
	       notify.sendNotify("爱企查E卡监控", notice);
        } catch (err) {
            console.log(err);
        }
        resolve();
