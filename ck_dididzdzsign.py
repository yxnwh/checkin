...
class DIDIdzdz-sign:
    ...
    #签到
    @staticmethod
    def sign(token,xpsid,wsgsig):
        try:
            wsgsig1 = wsgsig[random.randint (0, 25)]
            url = f'https://res.xiaojukeji.com/sigma/api/step/sign/v2?wsgsig={wsgsig1}'
            heards = {
                "user-agent": f"Mozilla/5.0 (iPhone; CPU iPhone OS 14_2_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 didi.passenger/6.2.4 FusionKit/1.2.20 OffMode/0",
            ...
            data = r'{"xbiz":"240300","prod_key":"ut-walk-bonus","xpsid":"' + f'{xpsid}' + r'","dchn":"aXxR1oB","xoid":"1d5c053d-4500-459c-b766-6c7099bb7112","uid":"283381191421562","xenv":"passenger","xspm_from":"","xpsid_root":"' + f'{xpsid}' + r'","xpsid_from":"","xpsid_share":"","version":1,"source_from":"app","city_id":"2","env":{"ticket":"' + f'{token}' + r'","cityId":"2","longitude":114.01024142795139,"latitude":22.638653700086806,"newAppid":10000,"isHitButton":true,"ddfp":"eb86143f9b73dd5478f66115a1fe74ba","deviceId":"eb86143f9b73dd5478f66115a1fe74ba","appVersion":"6.2.4","userAgent":"Mozilla/5.0 (iPhone; CPU iPhone OS 14_2_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 didi.passenger/6.2.4 FusionKit/1.2.20 OffMode/0","fromChannel":"1"},"ticketAppid":"30004"}'
    ...
    def main(self):
        ...
        for check_item in self.check_items:
            ...
            msg = (
                f"账号 {i}\n------ 滴滴多走多赚-签到开始------\n"
                + self.sign(token=token,xpsid_root=xpsid_root,wsgsig=wsgsig)
                + "\n------ 滴滴多走多赚-签到结束------"
            )
            i += 1
            msg_all += msg + "\n\n"
